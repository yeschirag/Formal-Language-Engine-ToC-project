...
/**
 * Panel that displays DFA conversion results (transition table + graph).
 */
export default function DFAConversionPanel({ automaton }) {
  const transitionRows = useMemo(() => {
    if (!automaton) return [];
    const { states, alphabet, transitions, startState, acceptStates } = automaton;
  ...
  if (!automaton) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-2 text-muted-foreground min-h-[200px]">
        <span className="text-3xl" role="img" aria-label="Convert">⚙️</span>
        <span className="text-sm font-medium">Generate an ε-NFA first to see the DFA.</span>
      </div>
    );
  }
